...
- 管理 API（`/admin/tokens/issue`、`/admin/tokens/extend`、`/admin/tokens/revoke`、`/admin/tokens/revoked`、`/admin/tokens/cleanup`、`/admin/users`、`/admin/kubeconfigs/upload`）需管理员 JWT
- 管理员可通过 MCP Tool 或 CLI 签发/延期/撤销用户 Token 和集群权限；`user_id == 'admin'` 不纳入延期管理
